...
//getchar();
//printf("%d", matrix[i][j]);
int cont1 = 0; int somaCategoria[200];
//percorre categorias
for (index=0; index<numAppsDiferentes; index++) { //percorre apps diferentes
		
		somaCategoria[appsDiferentes[index].categoria]+=appsDiferentes[index].cont;
...
// Definicao da matriz bidimensional
   matriz = (int**) malloc (sizeof(int*) * numAppsDiferentes+2); 
   for (i=0; i<numAppsDiferentes+2; i++) 
	matriz[i] = (int*) malloc (sizeof(int) * numAppsDiferentes+2);

	for (a1=0; a1<numAppsDiferentes; a1++){
			for (a2=0; a2<numAppsDiferentes; a2++){
				matriz[a1][a2]=0;
			}
		}
...
int mainIndex=0;
 for (mainIndex=0; mainIndex<mostDownloaded;mainIndex++) { //mostDownloaded = number of USers
			//printf("[%d] ", mat[i].at(j)); 
			 cout << "\n " << mainIndex+1 << "\t"; 
			 ...
			 for (index=0; index<numAppsDiferentes; index++) {
			  ...
			  //cout << "travei1? "; 
					...
					{ 		//0 1 2 3 ou acabou  a CategoriaAtual		
						baixado = rand() %(numCategoriasDiferentes);//cout << "travei? "; //sao 16 categorias
						while (/*(c[baixado] ==0) || */(baixado == atualCategoria)) 
						{
							baixado  = rand() %(numCategoriasDiferentes);
					...
					for (index=0; index<numAppsDiferentes; index++) {
						if ((appsDiferentes[index].categoria == atualCategoria) /*&& (appsDiferentes[index].cont > 0)*/){
							...
							contAppDiferente =0;
							/* verifica se ele nao ja baixou esse app*/		
													
							for (l=1; l<j;l++) {
								if (idBaixadosPerUser[l]!= index) {
									contAppDiferente++;
							...
							if ((contAppDiferente == j-1) || (j==2)){ //ainda nao baixou esse app se for diferente de todos da lista q baixou ou for o primeiro a ser baixado
							
								//cout << " [ " << appsDiferentes[index].id  << "]";
								cout << index <<"\t";
								appsDiferentes[index].cont--;
								idBaixadosPerUser[j] = index;
								baixou=true;
								break;
							}
							//o app ja foi baixado.. tem que sortear outro
							else{
					...
					//idBaixadosPerUser[j] = appsDessaCategoria[aleatorio]; //indice do app Diferente baixado
					//cout << appsDessaCategoria[aleatorio] << "\t";
				}
				
				for (a1=2; a1<peruser; a1++){
					for (a2=2; a2<peruser; a2++){
						if (idBaixadosPerUser[a1]!=idBaixadosPerUser[a2]) matriz[idBaixadosPerUser[a1]][idBaixadosPerUser[a2]]++;
							else matriz[idBaixadosPerUser[a1]][idBaixadosPerUser[a2]]=0;
					}
				}
