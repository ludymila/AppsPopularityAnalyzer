...
istream& operator >> ( istream& ins, record_t& record )
  ...
  while (getline( ss, field, ',' ))
	...
	if (contField == 18){
		...
		else if ((field == "\"Arcade & Action\"") || (field == "\"Game Adventure\"")) categoria=1;
...
srand((unsigned)time(NULL));
int index;
int idBaixadosPerUser[80];
cout <<"numero app dif" <<numAppsDiferentes; 
cout <<"peruser:" <<peruser; //getchar();
 ...
 //getchar();
			//printf("[%d] ", mat[i].at(j)); 
			 ...
			 for (index=1; index<numAppsDiferentes; index++) {
				if ((appsDiferentes[index].categoria == 0) /*&& (appsDiferentes[index].cont > 0)*/){
					...
					idBaixadosPerUser[1] = appsDiferentes[index].id;
					break;
				}
			 }			 
			  ...
			  //cout << "travei1? "; 
					...
					bool baixou=false; int l; int contAppDiferente =0;
					baixou=false;
					for (index=0; index<numAppsDiferentes; index++) {
						/* percorre o vetor de apps ate achar a categoria sorteada, e ve se o contador de downloads ainda nao zerou*/
						if ((appsDiferentes[index].categoria == atualCategoria)/* && (appsDiferentes[index].cont > 0)*/){
							...
							//getchar();
							//contAppDiferente =0;
							if (contAppDiferente == j-1){ //ainda nao baixou esse app se for diferente de todos da lista q baixou
								cout << " [" << appsDiferentes[index].categoria << "]";
								cout << " [ " << appsDiferentes[index].id  << "]";
								cout << " [" << appsDiferentes[index].nome  << "]\n";
								appsDiferentes[index].cont--;
								idBaixadosPerUser[j] = appsDiferentes[index].id; baixou=true;
								break;
							}
							...
							else cout << " [" << appsDiferentes[index].nome  << "]";
							appsDiferentes[index].cont--;
							break;*/
						}
					...
					if (baixou==false) 
							j--; //desconsidera o j (contador do for que controla app baixado), pq o app nao foi baixado
