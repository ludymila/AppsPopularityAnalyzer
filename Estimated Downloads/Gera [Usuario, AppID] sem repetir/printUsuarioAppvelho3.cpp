...
// Definicao da matriz bidimensional
   matriz = (int**) malloc (sizeof(int*) * numAppsDiferentes); 
   for (i=0; i<numAppsDiferentes; i++) 
	matriz[i] = (int*) malloc (sizeof(int) * numAppsDiferentes);
...
for (k=0;k<numAppsDiferentes; k++){
 ...
 for (i=0; i<mostDownloaded;i++) { //mostDownloaded = number of USers
			 ...
			 for (index=0; index<numAppsDiferentes; index++) {
			  ...
			  //cout << "travei1? "; 
					...
					{ 		//0 1 2 3 ou acabou  a CategoriaAtual		
						baixado = rand() %(17);//cout << "travei? "; //sao 16 categorias
				...
				for (i1=2; i1<peruser; i1++){
					for (i2=2; i2<peruser; i2++){
						if (i1!=i2) matriz[i1][idBaixadosPerUser[i2]]++;
					}
				}
		...
		for (i1=2; i1<numAppsDiferentes; i1++){
			for (i2=2; i2<numAppsDiferentes; i2++){
				cout << " " << matriz[i1][idBaixadosPerUser[i2]]++;
			}
			cout << "\n";
		}
