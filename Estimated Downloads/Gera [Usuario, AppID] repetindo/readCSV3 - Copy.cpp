...
istream& operator >> ( istream& ins, record_t& record )
  ...
  while (getline( ss, field, ',' ))
	...
	if (contField == 12){
		...
		numberDownloads = numberDownloads/1000;
		//cout << "\nnum down:" << numberDownloads; 
		if (mostDownloaded ==0) {
				mostDownloaded = atoi(field.c_str()); 
				mostDownloaded = mostDownloaded/1000;
	...
	int i;
	 ...
	 if (numberApps==0) 
		{
			//cout << "\nentrei:";
			for (i=0; i< int(mostDownloaded/100); i++){
				mat[numberApps].at(i) = 1; //preenche com 1 todos os usuarios do que tem mais downloadsmd
			}
		}
	else 
	{
